...
const HistoryScreen = () => {
  ...
  const renderItem = ({ item }) => (
    <View
      ...
      testID={`transaction-item-${item.id}`} // Test ID único por item
    >
      <View style={styles.transactionHeader}>
        ...
        <Text
          style={[
            styles.points,
            { color: item.points > 0 ? theme.colors.primary : theme.colors.secondary },
          ]}
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      ...
      <FlatList
        ...
        testID="transaction-list" // Test ID para a lista
      />
    </View>
  );
};
...
const styles = StyleSheet.create({
  ...
  benefitName: {
    fontSize: 14,
    fontWeight: '500',
    flex: 1, // Mantém o texto dentro do container
    marginRight: 8, // Espaço entre texto e pontos
  },
  points: {
    fontSize: 14,
    fontWeight: 'bold',
